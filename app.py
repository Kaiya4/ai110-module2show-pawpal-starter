...
if st.button("Add task"):
    # Map the dropdown string to your Priority Enum
...
if st.button("Generate schedule"):
    ...
    if scheduler.daily_plan:
        for task in scheduler.daily_plan:
            st.write(f"`[{task.priority.name}]` **{task.pet_name}** | {task.name} ({task.duration} min)")
            
        st.success(scheduler.explain_plan())
    else:
        st.warning("No tasks scheduled. Check your available time or ensure tasks are added.")